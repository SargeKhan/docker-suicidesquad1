...
while true; do
    log=`sudo docker events --filter 'image=plumgrid2_node' --filter 'event=die' --filter 'event=stop' --since=5s --until=0s`
    echo "Checking after every 3 seconds if a contianer has stopped"
    if [ -z "$log" ]; then
        sleep 5
    else 
        echo "Someone is dead, Stopping rest of the containers"
        sudo docker stop $(sudo docker ps -q --filter ancestor=plumgrid2_node )
        exit 1
    fi
done
